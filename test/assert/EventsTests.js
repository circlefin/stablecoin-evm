...
async function run_tests(newToken, accounts) {
  ...
  it("et000 should check MinterConfigured event", async function () {
    let configureMinter = await token.configureMinter(
  ...
  it("et001 should check Mint/Transfer events", async function () {
    ...
    let mint = await token.mint(Accounts.arbitraryAccount, amount, {
  ...
  it("et003 should check MinterRemoved event", async function () {
    ...
    let minterRemovedEvent = await token.removeMinter(Accounts.minterAccount, {
      from: Accounts.masterMinterAccount,
    });
  ...
  it("et004 should check MasterMinterChanged event", async function () {
    let updateMasterMinter = await token.updateMasterMinter(
      Accounts.arbitraryAccount,
  ...
  it("et005 should check Blacklisted event", async function () {
    let blacklist = await token.blacklist(Accounts.arbitraryAccount, {
  ...
  it("et006 should check UnBlacklisted event", async function () {
    let unblacklist = await token.unBlacklist(Accounts.arbitraryAccount, {
      from: Accounts.blacklisterAccount,
    });
  ...
  it("et008 should check Upgraded event", async function () {
    ...
    let upgrade = await proxy.upgradeToAndCall(
  ...
  it("et009 should check AdminChanged event", async function () {
    let adminChanged = await proxy.changeAdmin(Accounts.arbitraryAccount, {
      from: Accounts.proxyOwnerAccount,
    });
  ...
  it("et010 should check OwnershipTransferred event", async function () {
    let transferOwnership = await token.transferOwnership(
      Accounts.arbitraryAccount,
      { from: Accounts.tokenOwnerAccount }
    );
  ...
  it("et011 should check Approval event", async function () {
    let approval = await token.approve(Accounts.arbitraryAccount2, amount, {
  ...
  it("et014 should check PauserChanged event", async function () {
    let updatePauser = await token.updatePauser(Accounts.arbitraryAccount, {
      from: Accounts.tokenOwnerAccount,
    });
  ...
  it("et015 should check Transfer event", async function () {
    ...
    let transfer = await token.transfer(Accounts.arbitraryAccount2, amount, {
      from: Accounts.arbitraryAccount,
    });
    checkTransferEvents(
      transfer,
  ...
  it("et016 should check Transfer event in transferFrom", async function () {
    ...
    await token.approve(Accounts.arbitraryAccount2, amount, {
      from: Accounts.arbitraryAccount,
    });
    let transferFrom = await token.transferFrom(
      Accounts.arbitraryAccount,
