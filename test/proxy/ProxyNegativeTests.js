...
async function run_tests(newToken, accounts) {
  ...
  it("nut002 should fail to switch adminAccount with non-adminAccount as caller", async function () {
    await expectRevert(
      proxy.changeAdmin(Accounts.masterMinterAccount, {
        from: Accounts.masterMinterAccount,
      })
    );
    assert.isTrue(
      addressEquals(
        await proxy.admin({ from: Accounts.proxyOwnerAccount }),
  ...
  it("nut003 should fail to upgradeTo to null contract address", async function () {
    ...
    await expectRevert(
      proxy.upgradeTo(zeroAddress, { from: Accounts.proxyOwnerAccount })
    );
  ...
  it("nut004 should fail to upgradeToAndCall to null contract address", async function () {
    ...
    await expectRevert(
      proxy.upgradeToAndCall(zeroAddress, initializeData, {
  ...
  it("nut006 should fail to call contract function with adminAccount", async function () {
    await expectRevert(
      token.allowance(Accounts.minterAccount, Accounts.arbitraryAccount, {
        from: Accounts.proxyOwnerAccount,
      })
    );
  ...
  it("nut008 shoud fail to update proxy storage if state-changing function called directly in FiatToken", async function () {
    ...
    assert.isTrue(
      addressEquals(await rawToken.pauser(), Accounts.pauserAccount)
    );
    await rawToken.updatePauser(Accounts.masterMinterAccount, {
      from: Accounts.tokenOwnerAccount,
    });
    assert.isTrue(
      addressEquals(await rawToken.pauser(), Accounts.masterMinterAccount)
    );
  ...
  it("nut009 should fail to call upgradeTo with non-adminAccount", async function () {
    var upgradedToken = await UpgradedFiatToken.new();
    await expectRevert(
      proxy.upgradeTo(upgradedToken.address, {
        from: Accounts.masterMinterAccount,
      })
    );
