...
d:
	forge script -vv --gas-estimate-multiplier $(GAS_MULTIPLIER) --rpc-url $(RPC_URL) -vvv

deploy-verify:
	forge script -vv --gas-estimate-multiplier $(GAS_MULTIPLIER) --rpc-url $(RPC_URL) --etherscan-api-key $(ETHERSCAN_API_KEY) --verifier $(VERIFIER) --verify --verifier-url $(VERIFIER_URL) --broadcast --slow

deploy:
	forge script -vv --gas-estimate-multiplier $(GAS_MULTIPLIER) --rpc-url $(RPC_URL) -vvv --broadcast --slow
