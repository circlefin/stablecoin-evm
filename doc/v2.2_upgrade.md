...
8. Verify that the list of accounts to blacklist is correct, and is set
   ...
   ```sh
   $ yarn execScript scripts/validateAccountsToBlacklist.js
      [--datasource-file-path=${path to JSON file}]
...
9. Using the Admin Key, transfer the proxy admin role to the `V2_2Upgrader`
   ...
   For mainnet upgrades, run the following command to validate the proxy admin
   is being assigned to the correct V2_2Upgrader contract address:

   ```sh
   $ yarn execScript scripts/validateChangeProxyAdminTx.js \
    --proxy-address=${Proxy contract address} \
    --upgrader-address=${V2.2 Upgrader contract address} \
    --rpc-url=${RPC url} \
    --signed-tx=${Signed changeAdmin transaction}
   ```
