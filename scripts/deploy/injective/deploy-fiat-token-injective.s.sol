...
/**
 * A utility script to directly deploy Fiat Token contract for Injective
 *
 * @dev This script deploys contracts but does NOT initialize them. Initialization must be done separately
 * using the TypeScript helper (initialize-injective.ts) because FiatTokenInjectiveV2_2.initialize()
 * calls the Injective bank precompile which doesn't exist in Foundry's simulation environment.
 *
 ...
 */
contract DeployFiatTokenInjective is Script {
    ...
    function setUp() public {
        ...
        impl = vm.envOr(
            "FIAT_TOKEN_INJECTIVE_IMPLEMENTATION_ADDRESS",
            address(0)
        );
        ...
        console.log("FIAT_TOKEN_INJECTIVE_IMPLEMENTATION_ADDRESS: '%s'", impl);
    ...
    function _deploy(
        address _impl
    ) internal returns (FiatTokenInjectiveV2_2, MasterMinter, FiatTokenProxy) {
        ...
        FiatTokenInjectiveV2_2 fiatTokenInjectiveV2_2;
        if (_impl == address(0)) {
            fiatTokenInjectiveV2_2 = new FiatTokenInjectiveV2_2();
        } else {
            fiatTokenInjectiveV2_2 = FiatTokenInjectiveV2_2(_impl);
        }

        FiatTokenProxy proxy = new FiatTokenProxy(
            address(fiatTokenInjectiveV2_2)
        );
        ...
        // NOTE: Initialization is NOT done here because FiatTokenInjectiveV2_2.initialize()
        // calls the Injective bank precompile (0x64) which doesn't exist in Foundry's simulation.
        // The initialization must be done separately using the TypeScript helper.
        //
        // FiatTokenInjectiveV2_2 proxyAsV2_2 = FiatTokenInjectiveV2_2(address(proxy));
        ...
        vm.stopBroadcast();
        return (fiatTokenInjectiveV2_2, masterMinter, proxy);
    }
    ...
    function deploy(
        address _impl
    ) external returns (FiatTokenInjectiveV2_2, MasterMinter, FiatTokenProxy) {
    ...
    function run()
        external
        returns (FiatTokenInjectiveV2_2, MasterMinter, FiatTokenProxy)
    {
        return _deploy(impl);
    }
}
