...
var UpgradedFiatTokenNewFieldsNewLogic = artifacts.require(
...
function checkMinterConfiguredEvent(
  ...
  assert.isTrue(
    addressEquals(configureMinterEvent.logs[0].args.minter, minter)
  );
  assert.equal(
    configureMinterEvent.logs[0].args.minterAllowedAmount,
    minterAllowedAmount
  );
}
...
function checkMinterRemovedEvent(minterRemovedEvent, minter) {
  ...
  assert.isTrue(
    addressEquals(minterRemovedEvent.logs[0].args.oldMinter, minter)
  );
}
...
function checkPauserChangedEvent(pauserChangedEvent, pauser) {
  ...
  assert.isTrue(
    addressEquals(pauserChangedEvent.logs[0].args.newAddress, pauser)
  );
}
...
function checkTransferOwnershipEvent(
  ...
  assert.isTrue(
    addressEquals(transferOwnershipEvent.logs[0].args.newOwner, newOwner)
  );
}

function checkUpdateMasterMinterEvent(
  checkUpdateMasterMinterEvent,
  newMasterMinter
) {
  assert.equal(
    checkUpdateMasterMinterEvent.logs[0].event,
    "MasterMinterChanged"
  );
  assert.isTrue(
    addressEquals(
      checkUpdateMasterMinterEvent.logs[0].args.newMasterMinter,
      newMasterMinter
    )
  );
}
...
function checkAdminChangedEvent(adminChangedEvent, previousAdmin, newAdmin) {
  ...
  assert.isTrue(
    addressEquals(adminChangedEvent.logs[0].args.previousAdmin, previousAdmin)
  );
  assert.isTrue(
    addressEquals(adminChangedEvent.logs[0].args.newAdmin, newAdmin)
  );
}
...
function checkUpgradeEvent(upgradeEvent, implementation) {
  ...
  assert.isTrue(
    addressEquals(upgradeEvent.logs[0].args.implementation, implementation)
  );
}

function checkTransferProxyOwnershipEvent(
  transferProxyOwnershipEvent,
  previousOwner,
  newOwner
) {
  assert.equal(
    transferProxyOwnershipEvent.logs[0].event,
    "ProxyOwnershipTransferred"
  );
  assert.isTrue(
    addressEquals(
      transferProxyOwnershipEvent.logs[0].args.previousOwner,
      previousOwner
    )
  );
  assert.isTrue(
    addressEquals(transferProxyOwnershipEvent.logs[0].args.newOwner, newOwner)
  );
}
...
async function checkVariables(_tokens, _customVars) {
  ...
  for (n = 0; n < numTokens; n++) {
    ...
    if (debugLogging) {
      ...
      console.log(
        util.inspect(actualState, { showHidden: false, depth: null })
      );
    }
    ...
    var accounts = Object.keys(Accounts).map(
    ...
    assert(
      balanceSum.cmp(totalSupply) == 0,
      "sum of balances is not equal to totalSupply"
    );
  }
}
...
async function setMinter(token, minter, amount) {
  let update = await token.configureMinter(minter, amount, {
...
async function blacklist(token, account) {
  let blacklist = await token.blacklist(account, {
...
async function unBlacklist(token, account) {
  let unblacklist = await token.unBlacklist(account, {
    from: Accounts.blacklisterAccount,
  });
...
async function setLongDecimalFeesTransferWithFees(
  ...
  assert.isTrue(
    balanceFeeAccount
      .sub(initialBalanceFeeAccount)
      .cmp(newBigNumber(feeAmount)) == 0
  );
}
...
async function getInitializedV1(token) {
  ...
  if (slot8DataLength == 3) {
    ...
    masterMinterAddress =
      "0x" +
      slot8Data
        .substring(masterMinterStart, masterMinterStart + 40)
        .toUpperCase();
